...
void sdl_window_plain::draw_goopax_impl(std::function<void(image_buffer<2, Eigen::Vector<Tuint8_t, 4>, true>& image)> func)
{
    ...
    std::array<unsigned int, 2> size = {(unsigned int)surface->w, (unsigned int)surface->h};
    {
        ...
        if (false)
        {
            // Mapping directly to surface pointer. Does not seem to work on apple."
            image_buffer<2, Eigen::Vector<Tuint8_t, 4>, true> image(device, size, BUFFER_WRITE_DISCARD, reinterpret_cast<Eigen::Vector<Tuint8_t, 4>*>(surface->pixels));
            func(image);
            device.wait_all();
        }
        else
